...
- 科学的方法論（仮説→反証→再現→結論）に準拠したパイプライン
...
| Challenger | Verifier | Integrator 判断 |
|-----------|----------|-----------------|
| confirmed | verified | confirmed（高信頼） |
| confirmed | weak_evidence | confirmed（注記付） |
| disputed | verified | needs_review（false negative 候補） |
| disputed | unverifiable | disputed |
| downgraded | verified | confirmed（原 severity 維持） |
| needs_context | verified | confirmed |
