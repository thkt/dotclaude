...
- Deciders: thkt
- Date: 2026-04-20
- Confidence: high — verified on tally HEAD~1..HEAD 2-run test, aggregation rate 3% → 33%

## Context and Problem Statement

Audit multi-run aggregation (introduced in commit b3ee9a6 with `--runs=N`) used strict `(file:line, category, reviewer)` as dedup key. Empirical verification on tally `HEAD~1..HEAD` with code-quality-reviewer × 2 runs showed only 1 of 30 findings merged (3% aggregation rate). Observed drift sources between runs:
...
- Integrator layer (progressive-integrator / evidence-integrator, ADR 0035) uses `file:line:category` dedup with severity preservation — aggregation before integrator must not destroy category information
