...
- test-generatorはUT/integration専用。E2Eにはagent-browserが必要でツール要件が異なる

きっかけ: Knight Rider Testingパターン（独立エージェントによるアプリ検証）の検討から、E2Eテスト生成の体系的不備が顕在化した。

## 決定

Spec駆動のE2Eテスト生成を /codeと /featureに統合する。専用agent `e2e-test-generator` を新設し、/e2eスキルは廃止する。
...
| B: e2e-test-generator 新設 | Spec 駆動の専用 agent + /e2e 廃止       | 採用 |
| C: test-generator に統合   | test-generator に agent-browser を追加  | 却下 |
...
- test-generatorのツール（Read/Write/Edit/Grep/Glob/LS）にagent-browserを追加するとツール表面が肥大化
- UTはRGRC（skip → activate）パターンだがE2Eはブラウザ操作が必要で実行モデルが異なる

### Approach B 採用理由

- Specの `Type: e2e` シナリオを入力として自動でPlaywrightテスト生成
- agent-browser依存をe2e-test-generatorに閉じ込め、test-generatorを汚染しない
- /codeにdev server検出を追加し、条件成立時のみspawn（オプトイン）
- /e2eの機能はe2e-test-generatorがSpec駆動で代替するため、手動スキルは不要
...
| `agents/generators/e2e-test-generator.md`                    | 新規作成（Spec 駆動 + agent-browser）                          |
...
変更なし: test-generator, test-quality-evaluator, generating-tdd-tests
...
- T-NNN体系を共有するためtest-quality-evaluatorが変更なしでE2Eを計測可能
- /e2e廃止でスキル数が減り管理コスト低下
- litmus（ADR-0028）がE2Eテストも品質検査対象にできる基盤が整う

### 悪い結果

- 新agent追加（e2e-test-generator）
