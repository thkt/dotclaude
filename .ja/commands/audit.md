...
Leader が全対象ファイルを読み、全ドメインを直接レビュー。出力: 発見事項 YAML
→ スナップショット保存 → 差分表示。
...
Medium で challenger/verifier を省略: 全レビュアーが同じファイルを読むため Leader が直接統合。Large
tier ではファイルルーティングでサブセット化するため独立した challenge/verification が不可欠。
...
| ファイルパターン                | レビュアー                                                      |
| ------------------------------- | --------------------------------------------------------------- |
| `*.sh`                          | security, silent-failure, code-quality, operational-readiness   |
| `*.ts, *.tsx, *.js`             | security, silent-failure, type-safety, code-quality,            |
|                                 | design-pattern, testability, performance, operational-readiness |
...
パスによる分類: `agents/**/*.md` → エージェント定義、`commands/**/*.md` または
`docs/**/*.md` → コマンド/ドキュメント、その他 `*.md`
→ コマンド/ドキュメント（デフォルト）。
...
PreToolUse(Skill) hook が `additionalContext` を注入した場合（例:
ADR-0013 に基づく
`claude-reviews`）、各ツールセクションを parse し、finding-schema.yaml の base
fields に従って `PF-{seq}` finding に変換する。
...
finding-schema.yaml の統合ルールを適用（同一パターン → 1 finding に統合）。
