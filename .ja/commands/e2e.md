...
└── [テスト名]/
    ...
    └── [テスト名].spec.ts-snapshots/  # ビジュアルリグレッションのベースライン（オプション）
```
...
### Step 4.5: ビジュアルリグレッション設定（オプション）

テスト生成後、Claudeがビジュアルリグレッションについて確認：
...
このE2Eテストにビジュアルリグレッションテストを有効にしますか？

メリット:
- 意図しないUI変更を自動検出
- ベースラインスクリーンショットをリポジトリに保存
- Claude Codeが過去のUI状態を参照可能

選択:
[Y] ビジュアルリグレッションを有効化
[N] スキップ（ドキュメント用スクリーンショットのみ）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**有効にした場合**、生成されるテストに `toHaveScreenshot()` が含まれる：
...
test("login", async ({ page }) => {
  ...
  // ビジュアルリグレッション: 最終状態をキャプチャ
  await expect(page).toHaveScreenshot("02-dashboard.png");
});
```

**閾値設定**（playwright.config.ts）：

```typescript
// 推奨設定（参照: https://zenn.dev/hiropon22/articles/1f49c0f62a0431）
...
| コマンド                                 | 目的                                         |
| ---------------------------------------- | -------------------------------------------- |
| `npx playwright test`                    | テスト実行、ベースラインと比較               |
| `npx playwright test --update-snapshots` | ベースラインを現在のスクリーンショットで更新 |

**ベースラインに差分がある場合**、Claudeは：
...
- [@../../skills/automating-browser/SKILL.md](../../skills/automating-browser/SKILL.md) - ブラウザ自動化ガイド
...
### ビジュアルリグレッションのベストプラクティス

**フォント安定化**（[hiropon22の記事](https://zenn.dev/hiropon22/articles/1f49c0f62a0431)より）：

環境差異（OS、ブラウザ）によりフォントレンダリングが変わる。安定化するには：

```typescript
// @fontsourceで環境間で一貫したフォントを使用
import "@fontsource/noto-sans-jp";
```

```css
/* またはBase64でフォントをCSSに埋め込み */
@font-face {
