---
description: コードベース分析から環境セットアップガイドを生成
...
コードベースを分析し、環境セットアップガイドドキュメントを自動生成。
...
検出結果をテンプレート（`~/.claude/skills/setting-up-docs/assets/setup-template.md`）に埋め込み、Markdownドキュメントを生成。

### フェーズ3: Markdownバリデーション
...
| 環境変数ファイルなし | 警告を表示して継続 |
| jq/yq未インストール | 一部機能をスキップ |

## 関連

- **関連コマンド**: `/docs:architecture`
