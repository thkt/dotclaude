...
class RenderTest(unittest.TestCase):
    ...
    def test_conformance_is_a_separate_section_not_in_blocking_count(self):
        # reviewer-conformance の issue 軸 findings は独自セクションに出し、blocking count
        # にはマージしない (blocking count は residual 件数のまま)。
        body = pr_body.render(FULL)
    ...
    def test_not_reaudited_warns_and_still_lists_residual(self):
        # Round-3 cap: 警告が出て、かつ未検証の findings が列挙される。
    ...
    def test_japanese_translates_prose_labels_but_keeps_github_keyword(self):
        # language: japanese は人間向けの section ラベルを翻訳する。GitHub の magic keyword
        # `Closes` と `/issue` コマンド名は verbatim のまま。
...
class CliTest(unittest.TestCase):
    ...
    def test_missing_required_key_fails_closed(self):
        # safety-critical key を落とした shipPayload は、それらしい「clean」body を描画せず
        # exit 1 して呼び出し側の && チェーンを中止させる。
