...
| disputed      | マーカー発見、トレードオフ文書化、レンズでスコープ外を示す、契約の読み違いで発見が実在しない、すでに低 FP | レポートから除外     |
...
Task 完了経由で 2 つのパートを返す。Markdown narrative (非権威な推論と根拠) の後に、単一の fenced JSON ブロック (権威ある decision フィールド) を続ける。integrator と /assert leader は verdict と severity を JSON ブロックからのみ読む。narrative で verdict や severity を再記しない。decision 値が 2 箇所に存在することが、この contract が塞ぐ cherry-pick 経路そのもの。
...
| Field                          | Type         | Rule                                                             |
| ------------------------------ | ------------ | ---------------------------------------------------------------- |
| challenges[].finding_id        | string       | 入力の finding_id と一致                                         |
...
| challenges[].adjusted_severity | enum or null | verdict = downgraded のときのみ設定。それ以外は null             |
| summary                        | object       | challenges から導出した count。人間向けで、第 2 のソースではない |

challenges がゼロは有効な結果でありエラーではない。`"challenges": []` を zeroed summary と共に出す。JSON ブロックの欠落や malformed はゼロ challenges ではない。consumer は 1 度だけ再実行し、その後 fail-close する。常にブロックを出す。
