...
機械的な型の正しさ (any 利用、strict モード、アサーション) はスコープ外で、gates のリンタが担う。本 reviewer はモデリング品質 (ドメイン概念の表現、カプセル化、不変条件) を見る。
...
`~/.claude/skills/audit/references/calibration-examples.md` の TD セクションを参照。
