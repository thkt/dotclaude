...
コード前に設計問題を検出。ゲートと優先度の定義は `formatting-audits` に集約。 このエージェントはチェックを実行して findings を発行する。

このエージェントは `/formatting-audits` 専用。`/audit` の reviewer プールには 含まれず、`finding-schema.md` には準拠しない — 下記の独自 `CON-*` / P0-P2 フォーマットを使う。
...
SOW → Spec → Test のチェーンが途切れないことを保証。現ドキュメントのみから 次ステップを心中試行し、失敗を記録。
...
NFR プローブ: NFR Target に対して FR 同じ3質問。Rationale 空欄検出は `validating-specs` Check 8 に委譲。
...
`validating-specs` スキルに委譲。CON-NNN findings は `validating-specs` で 割り当てられた優先度とともに findings テーブルに追加される。
