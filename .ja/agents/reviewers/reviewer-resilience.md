...
| 2     | エラーハンドリング       | 欠落リトライ、未処理の障害、握りつぶされたエラー、サイレントなデフォルト |
| 3     | 認証/データ整合性        | クロスユーザーデータアクセス、所有権チェックの欠落、カスケード副作用     |
...
| security   | 脅威アクターと攻撃ベクトル                     | アクターなしのインシデントシナリオ (DB タイムアウト、OOM) |
...
カテゴリ: auth / data / resource / cascade / infra / state。Severity は blast_radius (critical / high / medium / low) に置き換え。Extra フィールド: blast_radius が severity を置き換え、failure (何が壊れるか)、hypothesis (When X, system will Y)。
