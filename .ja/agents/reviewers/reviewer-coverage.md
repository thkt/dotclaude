...
| ゴール       | 説明                                                  |
| ------------ | ----------------------------------------------------- |
| 欠落検出     | 未テスト経路、欠けているエラー/エッジケース、否定分岐 |
| 品質チェック | 振る舞い vs 実装の結合、リグレッション リスク         |
| 具体的な修正 | "テストを増やす" ではなく具体的なテストケースを提案   |

## 姿勢

カバレッジは行ではなく振る舞いの問題である。SUT をモックする 100% 行カバーのテストは何も証明しない。未テスト経路、否定ケース、リグレッション リスクを探す。

推論内で禁止する表現。振る舞い契約を特定せずに "implementation might change" と書くこと。トリガーを示さずに "edge case is unlikely" と書くこと。
