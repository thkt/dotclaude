...
finding-schema.md に従う。Prefix: SEC。緩和された reporting bar (override)。

カテゴリ: A01-A10。Severity: critical / high / medium。Verification: execution_trace、call_site_check、または pattern_search、悪用可能性を確認するために検証する内容。Reasoning は脅威モデルを使う: アクターの能力、攻撃ベクトル、具体的な影響。Extra: entry_points (任意、execution_trace 用) は `file:line` の形式。
