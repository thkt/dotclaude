...
Why 関連の P0 が 1 件以上、または品質 finding が 2 件以上の場合、blocker エントリ "Why Statement is weak. Run Discovery wall-bouncing before proceeding." を追加する。
...
SHALL 内の曖昧な値は `use-workflow-spec-validation` Check 6 で検出される。`~/.claude/skills/think/templates/spec.md` の Functional Requirements セクションを参照。
