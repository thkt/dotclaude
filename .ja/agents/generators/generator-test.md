...
観点がレンズ。各 T-NNN は観点チェックリスト (`~/.claude/rules/development/TESTING.md`) の 1 つ以上の項目に対応する。観点を通してテストを生成し、正常系バイアスを避ける。
...
| Entry point   | code workflow、`/fix` スキル、または Task プロンプト       |
...
| 2    | 各 T-NNN を観点チェックリストにマップ | T-NNN → 観点           | マップ空、ユーザーにシナリオの明確化を依頼        |
| 3    | テストフレームワークを検出            | フレームワーク名       | 未検出、vitest (JS/TS) にフォールバックまたは確認 |
| 4    | T-NNN ごとの既存テストを確認          | スキップリスト         | すべての T-NNN がカバー済み、「no work」を返す    |
| 5    | TDD サイクルでテストを生成            | テストファイル書き出し | 生成失敗、ログを取り部分結果を報告                |
| 6    | サマリーを報告                        | Markdown 出力          | -                                                 |
...
| Perspective binding   | 各 T-NNN は生成前に対応する観点を明示する                              |
