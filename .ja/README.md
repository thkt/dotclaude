...
- plan: 計画と設計 (/outcome でアウトカム定義、/think で設計探索と SOW/Spec 生成、/research で技術調査、/probe でリポジトリ仕様抽出、/slice で計画を縦スライス issue に分割)、アーキテクチャエージェント付き
- build: 実装とテスト (/build で一気通貫、/code, /fix, /swarm を TDD/RGRC サイクルと品質ゲートで、/shake で flaky テスト検出、/prototype で使い捨てプロトタイプ)
- review: 品質とレビュー (/audit, /polish, /preview, /challenge, /assert)、専門 reviewer エージェント 21 種付き
- ship: Git とリリース (/commit, /checkout, /pr, /issue)
- toolkit: ユーティリティとパターン (/adr で意思決定記録、/adrift で ADR とコードの乖離検出、/census で未記録判断の発掘、/reflect でセッション振り返り)、フロントエンドパターン参照付き
