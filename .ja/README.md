...
- plan: 計画と設計 (/outcome でアウトカム定義、/think で設計探索と SOW/Spec 生成、/research で技術調査、/slice で計画を縦スライス issue に分割)、アーキテクチャエージェント付き
- build: 実装とテスト (/build で一気通貫、/code, /fix を TDD/RGRC サイクルと品質ゲートで、/shake で flaky テスト検出)
- review: 品質とレビュー (/audit, /polish, /preview, /challenge, /assert)、専門 reviewer エージェント 18 種付き
