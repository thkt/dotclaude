...
すべての skill は skills/ 直下に置く。Claude は SKILL.md を最初に読み、必要な時だけ references を読む。共有フラグメントは \_lib/ 直下に置く。
...
| `${CLAUDE_SKILL_DIR}/../<dir>/foo.md`         | クロス skill | ハーネスが展開し、Read が `..` を正規化する。skills 横断の `_lib/` 共有に使う |
...
| 軸                         | 合格条件                                                        | 不合格サイン                                                |
| -------------------------- | --------------------------------------------------------------- | ----------------------------------------------------------- |
| 単一責務                   | 1 skill 1 タスク。無関係な 2 つ目のトリガーは分割サイン         | description が無関係な capability を 2 つ以上連結           |
| description の識別性       | 1 文目が具体的な動詞と対象で capability を列挙する              | helps with や manages など多くの skill に当てはまる汎用動詞 |
| 命令形                     | 本文が agent に直接命令する                                     | skill が何をするかの受動的説明                              |
| 検証可能な完了             | step が検証可能な条件と明示的な stop point で終わる             | done 状態が無い、または検証条件のない確認指示止まり         |
