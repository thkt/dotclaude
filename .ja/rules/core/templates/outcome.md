...
`.claude/OUTCOME.md` の stub テンプレートとプロジェクト形態の例。`rules/core/OUTCOME.md` の不在時の振る舞いから参照される。stub 生成時のみ消費されるため常時ロードから外し、フローが明示的に読む。
...
- 開発者は識別子を正確に知らなくても concept 関連コードを発見できる
- 開発者は別語で再検索せずに、検索結果に基づいて行動する

### Indicators

| Indicator | 値                               | 裏付ける Behavior          |
| --------- | -------------------------------- | -------------------------- |
| Time      | 10MB 以下のリポで検索が 2 秒以内 | 開発者が検索途中で諦めない |
```
