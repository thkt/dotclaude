...
| ----- | ------------------- | -------------------------------- | ------------------------ |
| 1     | Discovery           | コンテキストスキャン → PREFLIGHT | 不明点・推論を解消       |
| 2     | Design              | Skill: /think                    | 設計承認                 |
| 3     | Implementation      | Skill: /code                     | -                        |
| 4     | Quality Loop        | /audit → /fix loop (max 3)       | 残課題のみ               |
| 5     | Visual Verification | ブラウザチェック (UI 変更時)     | ビジュアル承認           |
| 6     | Summary             | AC カバレッジ + scope 報告       | 完了                     |

### Phase 1: Discovery

1. コンテキストスキャン: CLAUDE.md, package.json, Cargo.toml など
2. `$ARGUMENTS` が空 → コンテキスト適応の選択肢で AskUserQuestion
3. PREFLIGHT を実行
4. 推論や不明点を解消
5. 早期終了: 対象ファイルが 2 以下 → `/code` を提案 (Phase 2-6 をスキップ)
...
| Step | 動作                                 | 終了条件                   |
| ---- | ------------------------------------ | -------------------------- |
| 1    | Skill: /audit (git で changed files) | 0 critical/high → Finalize |
| 2    | critical/high それぞれに Skill: /fix | → Step 3                   |
| 3    | iteration をインクリメント (max 3)   | 上限到達 → Finalize        |
...
script 値に明記があれば port を抽出する (`--port`, `-p`, `PORT=`)。
...
8. "Request fix" の場合 → Phase 4 Loop Step 2 に戻る
9. `agent-browser close`

### Phase 6: Summary

サマリーを提示する。AC カバレッジは /code Quality Gates で既に検証済み。
...
実装の根拠: `git diff main...HEAD --name-only` が SOW スコープのファイルを示す。
...
| /code Quality Gates が AC 未達を表示 | Phase 3 か 4 への再入を提案       |
