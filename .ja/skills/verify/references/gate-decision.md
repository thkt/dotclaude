...
/verify の Ready/NotReady バイナリ判定。reconciled evidence から導出、数値スコアなし。

Priority / Gate / Finding Format の正典定義: `formatting-audits`。
...
Zero-tolerance on findings: /verify は独立した outcome check。1件の reconciled finding
...
過去の /verify report は Trust Score (NN/100) 形式の可能性あり。Diff from Previous
セクションはこれを検出して `Legacy format — diff skipped` を出す。
`formatting-audits` Legacy Format Handling 参照。
