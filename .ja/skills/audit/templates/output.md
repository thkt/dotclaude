...
出力は `snapshot.json` (canonical source。ADR 0047 と `references/snapshot-schema.md` を参照) から render される。以下のすべての値は snapshot field に traceback できる。snapshot 起源のないデータを出力に出さない。Leader が render を orchestrate し、integrator が snapshot を埋める。
