...
finding-schema.md の Consolidation ルール (同パターン → 単一 finding) を PF finding 内のみで適用する。PF と Wave 1 は別扱い (下記 PF Findings の Pipeline 扱い)。
...
### Wave 1 との cross-reference

PF finding と同じ `file:line` (overlap ±3、`SKILL.md` の Multi-run 集約と同一 tolerance) に Wave 1 finding がある場合、integrator は Wave 1 finding の evidence に cross-reference を追記する。
