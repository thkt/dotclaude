...
`incomplete-contract` は、コードのコメントが「何が真か」を述べる一方で「何が真であり続けるべきか」を述べていない finding。読み手が「この状態は維持すべき」と推測することに依存し、セキュリティ不変条件や設計根拠で頻出する。欠けた forward-looking なルールは ADR でしか補えないため、`documented?` の値に関わらず強い ADR 候補とする。
...
`critic-design` は各候補に上記観点で挑み、下表のいずれかを返す。最終候補リストは `keep` と、吸収先 ADR を明示した `downgrade` で構成し、`drop` は除外する。

| 判定        | 意味                                                                                                          |
| ----------- | ------------------------------------------------------------------------------------------------------------- |
| `keep`      | ADR 化に値する。単独 ADR または関連候補と統合して起票                                                         |
| `downgrade` | 単独 ADR ではなく、関連 ADR 内のセクションに吸収またはコメント強化                                            |
| `drop`      | ADR 不要。設定 / コメント / テストで既に守られている、コストが価値を上回る、または bug (bug-fix の後続に回す) |
