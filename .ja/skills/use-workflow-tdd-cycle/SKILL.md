...
| 文脈                      | 理由                                  |
| ------------------------- | ------------------------------------- |
| プロトタイプ (使い捨て)   | 廃棄される可能性が高い、コスト > 効果 |
...
| Refactor | 綺麗なコード   | テストを green に保つ。読みやすくなる場合に限り縮める (~/.claude/rules/PRINCIPLES.md) | 振る舞いを変更してしまう。難読化する圧縮 |
...
| 2   | テストが構造的なアサーション (データ形状、シグネチャ) だけに退化する |
| 3   | 挙動変化への感度が落ちる (壊れていてもパスし、正しくても落ちる)      |
| 4   | 実装の知見がテスト構造を導くのではなく、テスト構造に追従する         |

mattpocock/skills の `tdd` SKILL.md に基づく。
...
bug-driven フロー (`/fix`) では、再現手順が spec の役割を果たす。
...
システム境界で mock する。外部 API、データベース、ファイルシステム、ネットワーク、非決定的な依存 (時刻、乱数)、2 分サイクルを阻害する遅い依存。
