...
`{...}` は生成時に内容へ置き換える。`(任意)` のセクションは書くことがなければ見出しごと省略する。確定の項目 (ask, AC, 決定済みの制約) は無印、仮の項目 (AI が推論した HOW、未決定の判断) は `(仮: <着手時のアクション>)` を付ける (基準は SKILL.md § 確信度マーキング)。
...
- {方針を選んだ理由 (任意): `ネイティブ <dialog> を採用。フォーカス管理が標準で付くため (仮: 着手時に判断)`}
