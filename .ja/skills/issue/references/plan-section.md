...
preconditions と backlog_candidates は think skill が返す構造化 plan の必須フィールドではなく、Plan 節から build workflow が拾う追加情報。抽出は markdown の見出しと箇条書きの構造のみに依存し、特別なマーカーやコメント記法を要求しない。
