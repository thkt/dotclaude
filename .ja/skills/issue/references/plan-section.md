...
U-NNN は 001 からの連番で plan 内一意。T-NNN は plan 全体で一意 (unit ごとに振り直さない)。unit は実装順に並べ、並び順がそのまま実装順になる。
...
build workflow は Plan 節を LLM 抽出で自前 schema (build.js の EXTRACT_SCHEMA) に写し、fail-close の validate と U-NNN / T-NNN id の決定論クロスチェックで欠落と捏造を止める。書き手が守るのはフィールドの語彙でなくこの骨格で、見出しと箇条書きの安定が抽出の安定を担う。機械用の隠し block は置かない。
