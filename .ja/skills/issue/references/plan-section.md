...
build workflow は Plan 節を LLM 抽出で自前 schema (build.js の EXTRACT_SCHEMA) に写し、fail-close の validate と U-NNN / T-NNN id の決定論クロスチェックで欠落と捏造を止める。書き手が守るのはフィールドの語彙でなくこの骨格で、見出しと箇条書きの安定が抽出の安定を担う。
