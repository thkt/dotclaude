...
ユーザーが決めた要件は無印。ユーザーが未決定のまま残した判断と未検証の事実だけに `(仮: <着手時のアクション>)` をインラインで付け、特定行に紐づかない issue レベルの前提は Premises セクションに置く。マーカー語は settings.json の言語設定に従う (英語では `tentative`)。仮マークは build が assumptions として抽出し、draft PR の「前提 (veto 対象)」としてユーザーが覆せる対象になる。不確かな HOW は仮マークで書き残すのでなく、書かない。
