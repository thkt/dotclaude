...
/think の plan があるときだけ実施し、無ければ節ごと省略する。plan は `.claude/workspace/planning/` の `*.plan.md` 下書きを正とし、issue のタイトルに一致する最新のものを Read する。下書きが無く会話に plan があるときはそれを使う。

1. 下書きの `## Plan` と `## Backlog candidates` をそのまま本文に貼り付ける。書式は /think が所有し、`${CLAUDE_SKILL_DIR}/../think/references/plan-section.md` が定義する。会話の plan から写す場合も同じ書式に従う
2. 記載したパスの実在と行数規則を plan-section.md § 投稿前検証 の手順で検証する
3. `## Backlog candidates` が無ければ追記する。plan がスコープ外へ切り出した候補を列挙し、無ければ「なし」と書く
