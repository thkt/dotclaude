...
14. 本文を一時ファイルに書き出し、ラベルを付けて `gh issue create --title "<step 5 の title を verbatim>" --body-file <path>` で起票し、出力から Issue URL を取得。`--title` は challenge / research / think に渡したのと一字一句同一の文字列を使う (veto はこの title で evidence bundle を照合するため、ずれると起票が deny される) (§ ラベル。sandbox 互換で長い本文のエスケープを回避)
...
docs / chore と軽微 bug は challenge / research / think / plan 書き下ろし (step 9〜12) を飛ばし、代わりに header `判定スキップ` の AskUserQuestion で免除をユーザーに確認する (選択肢は検出した種別)。この確認が veto の skip 記録になり、adversarial フローを経ない create を通す。軽微 bug の該当例は typo 修正。非該当例は原因未特定の間欠 (intermittent) bug で、この場合は skip せず step 9〜12 を通常どおり通す。skip した issue は本文フッターに「軽微につき /fix で処理してもよい」と注記し、/fix への導線を示す。bug を軽微と判定するのは次の 3 基準を全て満たすときに限る。
...
feature / bug で skip 分岐非該当のとき、`Skill("research", "<issue タイトルを verbatim>\n\nIntent: <feature なら Feature planning、bug なら Bug investigation>。<issue 本文から導いた調査質問>")` を実行し、結果の要点を本文の根拠と think の入力に反映する。先頭に issue タイトルを verbatim で置き intent を明示するのは、research が explorer-feature を issue タイトル入りの prompt で起動する条件を満たすため (この explorer 記録が veto の research evidence になる)。stopped: unresearchable が返ったら、missing の各項目を AskUserQuestion で仮説付きに壁打ちして解消し、回答を反映して再実行する。この壁打ちは最大 2 回。上限に達しても解消しない項目は仮マークで本文に残し、扱いをユーザーに委ねる。

## Think

research の結果を添えて `Skill("think", "<タイトル + 本文>\n\nResearch: <research の要約>")` を実行し、構造化 plan を得る。先頭行を issue タイトルにし、think の verbatim title が plan-gate に流れるようにする (veto の plan evidence になる)。ready=false が返ったら、blockers の各項目を AskUserQuestion で仮説付きに問うて決着させ、再実行する。ready=false の決着は最大 2 回。上限に達しても ready にならない項目は仮マークで本文に残し、進め方をユーザーに委ねる。
