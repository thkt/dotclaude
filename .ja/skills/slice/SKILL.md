...
/slice は分解と依存順 publish が価値。1 件だけなら /issue を使う。slice が生む issue には `## Plan` がまだ無く、そのまま /build に渡すと build が ephemeral plan を自動生成して進む (人間レビュー未経由の assumption 付き)。plan の精度を上げたいスライスは、/think で plan を作り issue の `## Plan` 節へ書き足してから /build に渡す。既に構造化 plan を手元に持つなら /code を使う。
