...
`$ARGUMENTS` は単一の ADR ディレクトリパスを含み得る。空白を除去し、空文字列は自動検出 (Step 1) として扱う。非空ならリポジトリルート相対パスとして扱い、存在しなければ却下する。
...
## Step 3: Decision Outcome からのシンボル抽出
...
`python3 ${CLAUDE_SKILL_DIR}/../_lib/external-adr-refs.py --adr-dir <adr-dir> --json` を実行する。コードベースの `ADR-NNNN` 参照を Step 1 で検出した ADR ディレクトリの `NNNN-*.md` と突き合わせ、ローカルに無い ID を `external_refs` として返す。これを External ADR 依存として別セクションに記録する。メタ ADR が別リポジトリにあるがローカルに未昇格の場合を捕捉する。
...
| Per-ADR  | 各 ADR に Per-ADR Findings セクション (findings / "no drift" / "unverifiable") |
