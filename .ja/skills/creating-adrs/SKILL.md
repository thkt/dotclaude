...
| フェーズ        | 目的             | 主要アクション                                    |
| --------------- | ---------------- | ------------------------------------------------- |
| 1. 事前チェック | 重複を防止       | 既存ADRをチェック、命名検証、ディレクトリ確認     |
| 2. テンプレート | 構造を選択       | 決定タイプに基づいて4つのテンプレートから選択     |
| 3. 参照         | 証拠を収集       | プロジェクトドキュメント、Issue、PR、外部リソース |
| 4. 検証         | 品質保証         | 必須セクションを検証、完全性チェック              |
| 5. インデックス | ドキュメント更新 | ADRリスト自動生成、関連ADRをクロスリンク          |
| 6. リカバリー   | エラー処理       | パス自動修正、フォールバックテンプレート          |
