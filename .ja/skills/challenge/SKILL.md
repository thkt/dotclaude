...
6. 残差は可逆性と影響度で振り分ける。不可逆または影響大のものだけ AskUserQuestion で質問し (上限 7 問)、それ以外は best-guess を仮定として進め、出力の Why に veto 注記として明記する。subagent で飛ばした分岐と仮定で進めた残差も Why に残し、ユーザーが誤った判定を後から覆せるようにする
...
| Why              | 判定を支える事実分岐の証拠 (再現 / 反証) に、critic-design 2 体 (内部 / outcome) の判定と、best-guess で進めた残差の veto 注記 (可逆性付き) を織り込む。末尾に verdict が証拠範囲内の挑戦である旨を 1 行添える |
