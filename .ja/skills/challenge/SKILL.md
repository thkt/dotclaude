...
`$ARGUMENTS` に対象を受け取る。提案のファイルパスでも記述でもよい。空なら停止して対象の指定をユーザーに求め、会話から推測しない。複数行のときは先頭行を対象のタイトルとして扱い、critic-design の起動プロンプトへ言い換えずそのまま渡す。
...
2. 提案の論点を洗い出して振り分ける。証拠で答えが一つに決まる論点は事実、優先順位 / スコープ / トレードオフのように選択が要る論点は判断。振り分けは論点の性質で決め、advisor の自信度では決めない
...
| outcome_ref      | OUTCOME.md のパスと Outcome state / Non-goals / Constraints の要約。無ければ Phase 1 で確認した outcome |
...
1. critic-design を Task で 2 体並列に起動する。subagent_type は critic-design、run_in_background は false。一方は内部攻撃、もう一方に `outcome_ref` を渡して outcome 攻撃。outcome を確定できなければ outcome 攻撃は省略する。`ARCHITECTURE.md` 等があれば言及する。起動プロンプトに対象のタイトルをそのまま含め、結果は `{ verdict: "GO" | "NO-GO", weaknesses: string[] }` の JSON 1 object で返させる
2. 両者の完了を待ち、判定と弱点を突き合わせて重複を除く
3. 総合判定と Phase 1 の残差を VERDICT_SCHEMA `{ verdict, assumptions: [{ text, irreversible, underspecified }] }` に集約する。不可逆な仮定がある、仮定が 7 件を超える、内容が曖昧な仮定がある、のいずれかなら出来の良さに関わらず NO-GO へ降格し、手動で GO に戻さない
