...
証拠で proposal を自力 grill し、解けない残差だけ可逆性で振り分けてユーザーに返す。
...
2. 設計ツリーの分岐を列挙し、各分岐を事実 (証拠で一意に定まる) か選好 (優先順位 / scope 意図 / trade-off の選好) に分類する。分類は質問種別で機械的に切り、advisor の自信度では切らない。選好を事実扱いすると grill の核を潰す
3. ループを回す。subagent が事実分岐を並列検証 → advisor が統合 + 分類監査 + 次に解くべき証拠 gap を提示 → メインセッションが結果を整理し継続判断。これ以上の証拠で分類が変わらなくなったら break。上限 3 round、解けない分岐は残差に落とす
...
6. 不可逆または影響大の残差だけ AskUserQuestion で最小質問 (上限 7 問)。それ以外は best-guess を仮定として進め、出力の Assumptions に明記する。subagent で飛ばした分岐と仮定で進めた残差は出力に残し、ユーザーが誤判定を veto できる逃げ道とする
