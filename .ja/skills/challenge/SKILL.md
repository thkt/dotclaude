...
4. 総合判定と Phase 1 の残差を VERDICT_SCHEMA (`{ verdict, assumptions: [{ text, irreversible, underspecified }] }`) に集約する。不可逆な仮定がある / 仮定が 7 件を超える / 内容が曖昧な仮定 (underspecified) があるときは、出来の良さに関わらず NO-GO へ降格し、手動で GO に戻さない
