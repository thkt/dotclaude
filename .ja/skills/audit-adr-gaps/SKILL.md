...
| 7    | レポートを `docs/audit/<YYYY-MM-DD>-undocumented-decisions.md` に出力                       |
...
日付は `date -u +%Y-%m-%d` (UTC, ISO 形式) でタイムゾーン依存を排除。出力ディレクトリの存在を保証:
...
- レポートパス: `docs/audit/<YYYY-MM-DD>-undocumented-decisions.md`
...
- [ ] `docs/audit/<YYYY-MM-DD>-undocumented-decisions.md` が存在
