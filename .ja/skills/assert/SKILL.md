...
| Bootstrap fail / Step 1-3 (env)       | Phase 1c + 2a をスキップし、静的のみで続行。gate 分岐は phase-4.md § Bootstrap 失敗処理 を参照 |
| Bootstrap fail / Step 4 (build smoke) | Phase 1c + 2a をスキップ。gate 分岐は phase-4.md § Bootstrap 失敗処理 を参照                   |
| Bootstrap timeout (300s overall)      | Step 4 開始済みなら Step 4 fail、それ以外は Step 1-3 fail として扱う                           |
