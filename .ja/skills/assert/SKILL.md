...
## Pre-flight: OUTCOME.md を読む

Outcome-based assertion は assert 対象となる outcome 自体を必要とする。Phase 0 の前に `.claude/OUTCOME.md` を読み、このリポジトリにとって「完了」がどう見えるかを確定する。Behavior、Non-goals、Constraints は Phase 3 (Intent Assertion) と Phase 4 (Evidence Synthesis) で intent source として参照する。
...
| ----- | ------------------- | -------------------------- | ------------------- | -------- | --------------------------------------------------------- |
| -1    | OUTCOME.md 読込     | orchestrator (Read)        | sequential          | -        | 上記 Pre-flight セクション                                |
...
enhancer-evidence は権威ある JSON decision ブロックを発する。leader は gate と findings を jq で decode し (Phase 4 Gate Decode)、prose からは読まない。leader は decode した gate = Ready のときのみ完了メッセージで `gate = Ready` を明示する (Ready (caveat) や NotReady では明示しない)。これにより `/goal` evaluator が会話から完了を読み取る。Leader は decode した gate をそのまま中継し、再生成しない。
```
