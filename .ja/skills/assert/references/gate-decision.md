...
| 条件                  | アクション                                                      |
| --------------------- | --------------------------------------------------------------- |
| gate = Ready          | `<promise>PASS</promise>` を発し、ループ終了                    |
| gate = Ready (caveat) | Blockers (none) + caveat note を出力。反復継続。PASS は発しない |
| gate = NotReady       | Blockers と Fix 提案を出力。反復継続。PASS は発しない           |

Ready (caveat) で PASS を発しない理由: Ralph Loop は未テスト可能性のあるコードで終了すべきでない。継続することで、ユーザーが環境を復旧して再実行する余地、または手動でループを止めて caveat を意識的に受け入れる余地を残す。
