...
Integrator は単一の `issues` セットを構築する: `file:line` で重複除去、findings ごとにすべてのソースタグを保持 (例: `[challenger, adversarial]`)。統合セットが ${CLAUDE_SKILL_DIR}/references/gate-decision.md § Gate Rule のゲート判定を駆動する。
