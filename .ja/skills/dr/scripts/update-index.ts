...
//
// update-index.py の TypeScript 移植。header/entry の形は skills/_lib/harness_hash.ts、
// skills/dr/scripts/dr_common.ts、skills/dr/scripts/pre-check.ts、
...
//
// Contract: skills/dr/scripts/update-index.py の STATUS_SECTIONS、HEADER、FOOTER、parse_dr、
...
// update-index.py 自身の名前とは意図して異なる差分であり、
...
// update-index.py の STATUS_SECTIONS: By Status 見出しのテキストを、その DR の frontmatter の
// status がどの接頭辞に startWith するとその section に入るかというキーで引く。README の
// By Status 見出しと同じ proposed/accepted/deprecated/superseded/rejected の順で並ぶ。
...
// update-index.py の HEADER: DR List テーブルの冒頭を、ヘッダー区切り行までそのまま持つ --
// main() は各行のテキストをこの文字列の末尾改行の直後にそのまま続け、間に空行は入らない。
...
// update-index.py の FOOTER。{update_date} プレースホルダーは Python の
// str.format(update_date=...) と同じやり方で置き換える。最後の行の "update-index.ts" が
// update-index.py 自身の FOOTER との唯一の意図した違いである -- このファイルの header
// comment を参照。
...
/** update-index.py の parse_dr: frontmatter と最初の `# ` 見出しから (title, status, date) を
 * 得る -- frontmatter がどちらも持たないとき、status は "proposed" に、date は "Not set" に
 * 落ちる。これは Python の falsy な空文字列チェックがそうする `status or "proposed"` /
 * `date_str or "Not set"` のフォールバックと同じである。 */
...
/** update-index.py の `dr_dir.rglob("[0-9][0-9][0-9][0-9]-*.md")`: dir 配下のあらゆる深さに
 * あるファイルのうち、自身の名前が 4 桁の数字・ダッシュ・任意の文字列で .md に終わるものを
 * すべて、フルパスとして返す -- ソートは呼び出し側が行う (rglob が返す Path はソートするまで
 * 順序を持たない)。 */
