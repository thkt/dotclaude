...
各リポジトリの `.claude/OUTCOME.md` は以下の構造に従う。`{...}` は生成時に内容へ置き換える。Behavior は必須。Indicators は任意で、Behavior を裏付ける場合のみ書き、なければセクションごと省略する。記入例は `<!-- 例: ... -->` で添える。
...
- 禁止パターンを書いた AI エージェントはブロッキングシグナルを受け、同じ編集サイクル内でコードを修正する
...
- チームメンバーが手動のスプレッドシートコピーなしで毎週 key result を更新する
- ステークホルダーは owner に問い合わせずに組織全体の OKR 状態を読む
