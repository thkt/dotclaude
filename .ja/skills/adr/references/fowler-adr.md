...
Martin Fowler が bliki でまとめた ADR プラクティス。元々の概念は
Michael Nygard (2011) が提唱。
...
> ADR は、プロダクトやエコシステムに関する single な決定を記録し説明する
> 短いドキュメント。

基本 1 ページ、最大でも 2 ページ。記載する内容: 決定本体 / そのコンテキスト
/ 重要な波及効果。
...
| 目的                | 価値                                                     |
| ------------------- | -------------------------------------------------------- |
| 歴史的記録          | 数ヶ月後〜数年後に「なぜこう作られているか」を理解可能に |
...
- Markdown: コードと同じリポジトリで diff 可能に
...
ADR は Advice Process の中核でもある。書くこと自体が、チーム横断で
専門知と合意を引き出す手段になる（単なる記録ではない）。
