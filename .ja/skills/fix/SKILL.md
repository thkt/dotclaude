...
| 信頼度     | アクション                                       |
| ---------- | ------------------------------------------------ |
| [?] <50%   | 即座にエスカレート → `/research`                 |
| 50-69%     | ユーザーに確認: 防御的チェックで続行 or escalate |
| 複雑       | 複数ファイル → `/code`                           |
| 新スコープ | 機能 → `/think`                                  |
...
| 修正3回以上失敗             | 停止。フルコンテキストで `/research` にエスカレート |

≥3ルール:
3回の異なる修正試行が失敗した場合、問題はローカルなバグではなくアーキテクチャ的な可能性が高い。エスカレーションなしに4回目を試みてはならない。
