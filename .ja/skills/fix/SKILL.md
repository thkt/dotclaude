...
1. `Skill("use-context-root-cause-analysis")` を起動して 5 Whys を実行する。Finding ID または Finding 直接入力経由なら、finding の file:line と summary (snapshot があれば root cause も) を 5 Whys の起点として渡す。Symptom / Root cause / Pattern を出力する。
