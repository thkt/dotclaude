...
// TypeScript port of skills/dr/scripts/dr_common.py's shared helpers, in the same shape
// skills/_lib/harness_hash.ts uses (named exports, no default export). This module carries no
// CLI entry of its own -- pre-check.py, validate-dr.py, and update-index.py each import it, and
// their TS ports (later units) will do the same -- so it ships shebang-less at mode 100644,
// unlike the skills-CLI shebang + 100755 the scripts that call it carry.
//
// Contract: skills/dr/scripts/dr_common.py's fail, resolve_dr_dir, guard_skill_dir, and
...
//
// resolveDrDir is a pure function here, unlike resolve_dr_dir.py, which calls fail() itself
// on a non-zero git exit: env, the CLI argument, and the git-toplevel spawn result all arrive
// as explicit parameters, and a caller that finds none of the three decides what to do with
// the null it gets back. That keeps the git spawn and the process.exit side effect in the CLI
...
/** Writes each line to stderr and exits the process with status 1, mirroring
 * dr_common.py's fail(*lines): print(*lines, sep="\n", file=sys.stderr); sys.exit(1). */
 ...
 * Record archive), mirroring dr_common.py's guard_skill_dir. Returns normally otherwise. */
...
/** Splits text into (frontmatter lines, body lines) at a --- delimiter pair, mirroring
 * dr_common.py's split_frontmatter: only a --- on line 1 opens the fence, so a --- anywhere
 * in the body of a file that never opened one is never mistaken for a delimiter. */
