...
6. Sort the residual by reversibility and impact. Ask via AskUserQuestion only the irreversible or high-impact ones (cap 7). For the rest, proceed on the best-guess as a stated assumption logged in the output Why. Keep the branches skipped via subagent and the residuals advanced on assumption in the Why, leaving the user a way to veto a misjudgement
...
| Why              | The fact-branch evidence behind the verdict (reproduction / refutation), woven together with the two critic-design verdicts (internal / outcome) and the residuals advanced on a best-guess with their reversibility (the user's veto targets). Close with a one-line note that the verdict is a challenge within evidence range |
