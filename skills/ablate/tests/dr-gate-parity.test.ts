...
// names). Value parity is dr-gate.test.ts's and dr_gate_test.py's job; this test never calls
// gate, only reads which names each side declares. dr_gate.py stays live as report.py's import
// source until #646 retires it; this test retires with that slice.
//
// The python driver reads dr_gate.py's own AST rather than importing it and calling dir():
// `import re`, `from pathlib import Path`, and `from verdict import DELETE_CANDIDATE` all bind
// names into dr_gate.py's namespace that dir() would report as public, but dr_gate.py's text
// never declares any of them as its own -- dr_gate.ts imports the same three kinds of names
// (readFileSync/globSync/join, DELETE_CANDIDATE) without re-exporting them (see dr_gate.ts's
// header), so a dir()-based comparison would report names neither side actually added or
// dropped. Reading the source's own top-level Assign/AnnAssign/FunctionDef/ClassDef targets
// sidesteps that: a name reaches this comparison only when dr_gate.py's text itself declares
// it, which is the "look at names, not values" contract.
...
test("T-345 dr_gate exports the same set of public names on both sides", () => {
  const pyNames = pythonPublicNames("dr_gate");
