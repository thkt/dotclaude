...
| `skills/use-context-reviewer-readability/references/ai-antipatterns.md`    | skill-reference | `T-strategy-static` | Review code that defines a Strategy interface with multiple implementing classes to switch between calculation branches that never vary at runtime |
| `skills/use-context-reviewer-readability/references/control-flow.md`       | skill-reference | `T-branch-lookup`   | Review code with six or more sequential `if (code === ...) return ...` branches switching on one discrete value                                    |
| `skills/use-context-reviewer-testability/references/pure-functions.md`     | skill-reference | `T-inplace-mutate`  | Review a function that mutates an array or object parameter in place, such as `cart.push(item)`, instead of returning a new value                  |
| `skills/use-context-root-cause-analysis/references/hypothesis-examples.md` | skill-reference | `T-two-hypotheses`  | Root-cause a bug where only two hypotheses survive brainstorming and the diff is not yet narrow enough to produce a third                          |
| `skills/use-context-reviewer-silence/references/detection-patterns.md`     | skill-reference | `T-fire-forget`     | Review code that calls an async function such as `fetchData()` with no `await`, `.then()`, or `.catch()`                                           |
