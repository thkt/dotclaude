...
// Differential tests for hooks/_lib/textlint.ts against hooks/_lib/textlint.py (unit U-002,
// docs/decisions/0112-adopt-typescript-for-helper-scripts.md): for the same PATH and the same
// edited file, lint() must invoke the same runner + textlint command and read back the same
// result, and fix() must stay silent under the same conditions. Both modules resolve CONFIG
// from their own file's location (the real repo's .textlintrc.json), so this test controls the
// runner instead: a stub `bun` on PATH plays the role hooks/edit/tests/textlint_fix_test.py's
// test_graceful_skip_no_textlint and hooks/_lib/tests/rust-target.test.ts's installStubCargo
// already play for this codebase's binary-resolution tests -- shutil.which and its TS mirror
// both search PATH, so replacing PATH is what controls which runner (if any) they find.
...
/** A one-shot driver rather than a CLI: textlint.py has no __main__, so this is the smallest
 * way to call its lint/fix from outside the process. Mirrors hook-payload-parity.test.ts's
 ...
 * <config>` -- the exact argv textlint.py's _run builds -- with a fixed line naming the path
 * it was given, and returns that directory. */
...
test("T-379 a lint run reports its findings in the shape the python version produced", () => {
  ...
  assert.equal(tsResult, pyResult, "textlint.ts and textlint.py must read the same result");
});
...
test("T-380 an absent textlint binary leaves the hook silent", () => {
  ...
  assert.equal(pyResult, "", "textlint.py: an absent runner must read as no findings");
