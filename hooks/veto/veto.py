...
fail-closed の境界は gate / plan-gate / verdict-gate (parse 失敗は deny / exit 1)。
...
def title_arg(argv):
    """argv から --title <value> を取り出す。無ければ ""。"""
    try:
        i = argv.index("--title")
        return argv[i + 1]
...
def is_gh_issue_create(cmd):
    ...
    return re.search(r"(^|[\s;&|(])gh\s+issue\s+create(\s|$)", str(cmd or "")) is not None
...
def store_dir():
    ...
    return Path(os.environ.get("VETO_HOME") or Path.home() / ".claude" / "state" / "veto")
...
def evaluate(records, ctx):
    ...
    consumed_bundle = sum(
        1
        for r in sess
        if r.get("kind") == "consumed" and r.get("via") == "bundle" and r.get("normalized_title") == nt
    )
    skip_count = sum(1 for r in sess if r.get("kind") == "skip")
    consumed_skip = sum(1 for r in sess if r.get("kind") == "consumed" and r.get("via") == "skip")

    # この title の evidence bundle: challenge GO + plan ready。どちらも同期 gate script
    # (verdict-gate / plan-gate) の出力を Bash PostToolUse で recorder が捕捉したもの。
    verdict_go = any(
        r.get("kind") == "verdict" and r.get("normalized_title") == nt and r.get("verdict") == "GO"
        for r in sess
    )
    plan_ready = any(
        r.get("kind") == "plan" and r.get("normalized_title") == nt and r.get("ready") is True
        for r in sess
    )

    bundle_complete = verdict_go and plan_ready
...
def validate_plan(plan):
    ...
    units = [u if isinstance(u, dict) else {"id": f"units[{i}]"} for i, u in enumerate(units)]
    if not units:
    ...
    for i, u in enumerate(units):
        tests = u.get("tests") if isinstance(u.get("tests"), list) else []
        tests = [t if isinstance(t, dict) else {"id": f"units[{i}].tests[{j}]"} for j, t in enumerate(tests)]
        files = u.get("files") if isinstance(u.get("files"), list) else []
        depends_on = u.get("depends_on") if isinstance(u.get("depends_on"), list) else []
        uid = u.get("id")
        if not tests:
    ...
    def visit(uid, path):
        ...
        deps = u.get("depends_on") if u and isinstance(u.get("depends_on"), list) else []
        for d in deps:
            visit(d, path + [uid])
        state[uid] = "done"
...
def cmd_verdict_gate(argv):
    ...
    if inp.get("verdict") not in ("GO", "NO-GO"):
        sys.exit("verdict-gate: verdict field missing or not one of GO / NO-GO (VERDICT_SCHEMA mismatch)")

    assumptions = inp.get("assumptions") if isinstance(inp.get("assumptions"), list) else []
    warnings = [f"assumptions[{i}] is not an object" for i, a in enumerate(assumptions) if not isinstance(a, dict)]
    reasons = []
    if inp["verdict"] == "GO":
...
def cmd_gate():
    ...
    def deny(payload, title, reason):
        ...
        try:
            ...
            print(f"veto: failed to append deny record: {type(e).__name__}: {e}", file=sys.stderr)
    ...
    if not nt:
        deny(payload, "", "gh issue create without an extractable --title; cannot bind evidence")
    ...
    if result["decision"] == "deny":
        deny(payload, nt, f"no evidence bundle for this issue title ({', '.join(result['reasons'])})")
...
def cmd_record(kind):
    ...
    if kind == "bash":
        ...
        gate_run = re.search(r"veto\.py[\"']?\s+(verdict-gate|plan-gate)\b", cmd)
        if gate_run:
            try:
                out = json.loads(stdout.strip().split("\n")[-1] or "{}")
            except json.JSONDecodeError:
                return
            if gate_run.group(1) == "verdict-gate" and out.get("verdict") in ("GO", "NO-GO"):
                append_record({
                    "kind": "verdict",
                    "ts": now(),
            ...
            elif gate_run.group(1) == "plan-gate" and isinstance(out.get("ready"), bool):
                append_record({
                    "kind": "plan",
                    "ts": now(),
                    "session_id": session_id,
        ...
        # (c) 成功した gh issue create -> 使った bundle / skip を消費 (single-use)。
        ...
        if is_gh_issue_create(cmd) and not payload.get("agent_id"):
            ...
            if via:
                append_record({
                    ...
                    "session_id": session_id,
                    "via": via,
                    "normalized_title": ctx["normalized_title"] if via == "bundle" else None,
                })
        return
    ...
    if kind == "skip":
        ...
        skip_q = next((q for q in questions if q and q.get("header") == "判定スキップ"), None)
        if not skip_q:
            return
...
def main():
    ...
    if sub == "gate":
        ...
        try:
            ...
            print(f"veto: record failed, evidence not persisted: {type(e).__name__}: {e}", file=sys.stderr)
        sys.exit(0)
    elif sub in ("plan-gate", "verdict-gate"):
        try:
            (cmd_plan_gate if sub == "plan-gate" else cmd_verdict_gate)(sys.argv[2:])
        except Exception as e:
            # parse_json_object が拾わないコード異常も生 traceback でなく構造化した
            # fail-closed (exit 1 + 理由) にする。
            print(f"veto: {sub} errored: {type(e).__name__}: {e} (fail-closed)", file=sys.stderr)
            sys.exit(1)
    else:
        print(f"veto: unknown subcommand {sub!r} (gate | record | plan-gate | verdict-gate)", file=sys.stderr)
        sys.exit(1)
