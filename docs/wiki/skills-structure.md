...
| 対象              | 要求                                                | 満たさないときの挙動                                                 |
| ----------------- | --------------------------------------------------- | -------------------------------------------------------------------- |
| テストの言語側    | 英語側にのみ置く。`.ja/skills/` 配下のテストは 0 件 | `.ja/` に置いたテストは走らない。CI の Python tests は `find` で `skills` を起点にし、Node tests は `skills/**/tests/*.test.{js,ts}` の明示 glob で拾うため、いずれも `.ja/` 配下は対象に入らない |
