...
- skill の scripts は他の skill から `${CLAUDE_SKILL_DIR}/../<skill>/scripts/<file>` で呼べる。実在する経路は 3 本で、`issue/validate-issue-body.ts`、`research/find-prior-research.py`、`scribe/find_wiki_rule.py` が呼ばれる側になる
