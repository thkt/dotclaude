...
ページ化前の候補置き場。1 行は「内容 1 行 + 根拠」で、根拠は PR/issue 由来なら #番号、調査原本なら `[research:name.md](../research/name.md)` と書く。旧 `(research)` は原本未特定として残し、推測でリンクしない。同じ原本の別配置や改訂は独立した根拠に数えない。
