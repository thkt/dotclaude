...
1. 使用実績を計測して根拠表を作る。30日 spawn 回数 / audit findings 件数 / lint 代替可否 / セッションログ grep (#135 #138 #140 #147)
2. EN と .ja の両ツリー + docs (COMMANDS.md / SKILLS_AGENTS.md) を一括更新し、git ls-files / ugrep 全走査で残存参照ゼロを確認する (#165 #173 #180)
3. `.claude-plugin/marketplace.json` と `docs/SKILLS_AGENTS.md` の件数を同期し、`python3 -m json.tool` で JSON を検証する (#135 #141 #142 #146)
4. docs/decisions/ 内の言及は削除せず歴史記録として据え置く (#138 #140 #146)
5. PR 本文に弱点 (失われる検出層) と復活判断トリガーを明記する (#138 #142 #147)
6. 対象を扱う未着手 issue の有無を確認し、無効化されるものは close する (#107 #110 #112 #123 #125 #127)
