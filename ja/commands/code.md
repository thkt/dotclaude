...
**TDD/RGRCが主要な実装サイクルです**。test-generator拡張により：

- **Phase 0（準備）**: test-generatorがspec.mdからテストスキャフォールドを作成
...
1. テストが仕様と整合（Phase 0）
2. コードがまず動作（TDD Red-Green）
3. コードがクリーンで保守可能に（SOLID/DRYでリファクタ）

### 0. テスト準備（Phase 0 - インタラクティブテスト有効化）

**目的**: 仕様から**スキップ状態**のテストケースを生成し、ユーザー確認付きで1つずつ有効化して真のBaby Steps TDDを実現。

**使用タイミング**: spec.mdが存在し、テストシナリオを含む場合。
...
Task({
  ...
  description: "仕様からスキップ状態のテストを生成",
  prompt: `
機能: "${featureDescription}"
仕様: ${specContent}

スキップモードでテストを生成:
1. FR-xxx要件 → スキップ状態のテストケース [✓]
2. Given-When-Thenシナリオ → スキップ状態の実行可能テスト [✓]
...
| # | テスト名 | ステータス | 複雑度 |
|---|----------|------------|--------|
| 1 | ゼロ入力を処理 | ⏸️ SKIP | シンプル |
| 2 | 基本ケースを計算 | ⏸️ SKIP | 基本 |
| 3 | 閾値ロジックを適用 | ⏸️ SKIP | 中程度 |
| 4 | エッジケースを処理 | ⏸️ SKIP | 複雑 |

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

#### ステップ3: インタラクティブ有効化ループ

キュー内の各テストについて、有効化前にユーザーに確認：
...
#### ステップ4: 有効化してRedフェーズ開始

ユーザー確認（Y）時：

1. **スキップマーカーを削除**
2. **テスト実行** → 失敗を確認（Redフェーズ）
3. **Greenフェーズへ進む** → 最小限のコードを実装
4. **必要ならリファクタ**
5. **ステップ3に戻る** 次のテストへ
...
- **仕様駆動**: Given-When-Thenシナリオからテストを導出
- **アクション前の確認**: サプライズなし、意図的な進行
- **明確な進捗**: 常にサイクルのどこにいるか把握
