...
# 型安全性レビュー - TypeScriptベストプラクティス

目標: 最小限の型体操で最大の型安全性を実現。
...
| --- | --- | --- |
| 型カバレッジ | 95%+ | < 90% |
| any使用 | 0（正当な理由がある場合のみ） | > 5件 |
| 型アサーション | 最小限 | > 10件 |
| 暗黙のany | 0 | 1件以上 |
| Strictモード | 全て有効 | 無効項目あり |

## セクション別ロード

| セクション | ファイル | フォーカス | トリガー |
| --- | --- | --- | --- |
| カバレッジ | `references/type-coverage.md` | 明示的な型、anyの回避 | any, unknown, 型カバレッジ |
...
- [ ] すべての関数に明示的な戻り値型がある
- [ ] すべてのパラメータに型がある（暗黙のanyなし）
- [ ] すべてのデータ構造にInterface/type定義がある
- [ ] 明確な正当化なしに`any`を使用していない

### 型ガードとナローイング

- [ ] Union型に型述語（`is`関数）を使用
- [ ] 関連する型に判別可能なUnionを使用
- [ ] `never`による網羅性チェック
...
| TSに推論させる | 明らかなものを過度に型付けしない |
| ドキュメントとしての型 | 良い型はドキュメントとして機能 |
| unknownを優先 | より安全な処理のため`any`より`unknown`を使用 |
...
- [@~/.claude/skills/applying-code-principles/SKILL.md](../../../skills/applying-code-principles/SKILL.md) - 過度な型付けを避ける
...
- `type-safety-reviewer` - このスキルの主な利用者
