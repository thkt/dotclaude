...
Use explicit path prefixes to avoid ambiguity and reduce refactoring cost. Pick one grouping style per crate (`use std::{a, b}` vs separate lines); `rustfmt` enforces alphabetical order.
...
Prefer idiomatic Rust to reduce line count. `unwrap()` is acceptable in tests and where the invariant is proven by construction. Features tagged `(1.95+)` require `rust-version = "1.95"` in `Cargo.toml`; align MSRV with the newest feature the crate actually uses.
...
Split a module into a separate crate when any of the following apply.
