...
`license`: prefer `MIT OR Apache-2.0` (Rust-ecosystem default). Crate `name`: alphanumeric and `-` only; underscores in `name = "..."` are normalized to `-` at publish and search time.
...
Beyond `rustfmt` / `clippy`, these `cargo` subcommands close common gaps for AI-driven development. Install with `cargo install <tool>` or `cargo binstall <tool>` (faster via prebuilt binaries).
