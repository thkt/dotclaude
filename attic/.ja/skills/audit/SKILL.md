---
name: audit
description: 専門 reviewer エージェント群を adversarial challenge (critic-audit + critic-evidence) の下でオーケストレーションする。finding は集約する事実ではなく、論証すべき立場として扱う。クイック PR スクリーニングには使わない (/preview を使用)。
...
reviewer エージェントをオーケストレーションし、統合前に findings を critic-audit (challenge) と critic-evidence (verify) に通す。パイプラインは reviewer → challenge → verify → integrate であり、reviewer → aggregate ではない。各 finding は論証すべき立場であり、事実ではない。Finding schema は全 finding に `file:line` を要求する。根拠のないエントリは構造的に無効。
...
- `--runs=N` (任意、1-3, デフォルト 1)。N > 1 は確率的な findings ドリフトに対抗する multi-run 集約を起動。下記 Multi-run ポリシーを参照
- `--focus=<value>` (任意)。scope 指定時のデフォルト `all` を上書き。`security` / `performance` / `quality` / `a11y` / `all` を受け付ける
- `--no-limit` (任意)。ファイル数制限チェックをスキップ (下記ファイル数ポリシー)。CI 内 audit やブランチ全体レビューで意図が明示的なときに使う
...
#### Focus から Reviewer へのマッピング

ファイルルーティングがファイルパターンごとに reviewer を割り当てた後、Leader は結果を focus でフィルタする。focus セットに含まれる reviewer のみが実際に走る。`reviewer-causation` は Wave 1 セットに従う (逐次依存関係を参照)。`quality` または `all` で依存する上流 reviewer が含まれるときに走る。

フィルタルール。ファイルあたりの最終 reviewer セット = (ファイルルーティングでそのパターンに割り当てられた reviewer) ∩ (Focus reviewer)。あるファイルで交差が空のとき、その focus ではそのファイルをスキップする。
...
| all         | フィルタなし。ファイルルーティングの全 reviewer が走る                                                                                                                                                                                                                                                    |

### Multi-run ポリシー

reviewer の findings は同じ対象でも実行ごとにドリフトする。十分に裏付けられたカバレッジが重要なときは `--runs=2` または `--runs=3` を渡す。N > 1 のとき、Leader は Wave 1 (reviewer fan-out) を順次 N 回実行し、下記手順で findings を集約する。
...
4. メッセージが分岐したら最長を保持。検証が必要なら `messages: [...]` で両方保持

正規化なしの厳格キーマッチでは、reviewer がパス形式・行境界・category ラベルを実行ごとに変えるため、ほとんどの findings が unmerged のまま残る。三方向すべてに tolerance が必要。±3 チューニングの根拠 (経験則 ~3% → ~33% merge 率) は ${CLAUDE_SKILL_DIR}/references/aggregation-tuning.md にある。
...
| ---------------------- | ----------------------------------------------------------------- |
| scope 空、files ≤ 30   | 続行                                                              |
| scope 空、files > 30   | AskUserQuestion で 3 つの選択肢。下記 Narrow Scope の選択肢を参照 |
...
reviewer に割り当てられたファイル一覧が 10 ファイルを超えるとき、Leader は 10 件ずつのバッチに分割し、その reviewer に対し 1 バッチ 1 Task call を発行する。integrator は `(file, category, reviewer)` (Multi-run 集約と同一キー) でバッチ越しの findings を集約する。
...
| 2    | ファイルルーティング: ターゲットファイルを分類 → 該当 reviewer に割り当て                         |
...
opus + advisor + 深い分析は stream watchdog を超過するため、sonnet 上書きと advisor 禁止制約で reviewer の停滞を排除する。
